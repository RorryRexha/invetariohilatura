<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold text-gray-800 dark:text-gray-200">
            Gestión de Entradas
        </h2>
    </x-slot>

    {{-- Alpine + búsqueda backend --}}
    <div
        class="py-8"
        x-data="{ search: @js(request('search', '')) }"
    >

        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">

            <!-- HEADER -->
            <div class="mb-6 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">

                <h3 class="text-lg font-semibold text-gray-700 dark:text-gray-300">
                    Registro de entradas
                </h3>

                <!-- CONTROLES -->
                <div class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">

                    <!-- BUSCADOR -->
                    <input
                        type="text"
                        x-model="search"
                        @keydown.enter.prevent="window.location.href='{{ route('entradas.index') }}?search=' + encodeURIComponent(search)"
                        placeholder="Buscar folio, código, producto, orden..."
                        class="px-4 py-2 border rounded-lg shadow-sm w-full sm:w-72"
                    >

                    <!-- BUSCAR -->
                    <button
                        type="button"
                        @click="window.location.href='{{ route('entradas.index') }}?search=' + encodeURIComponent(search)"
                        class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded shadow"
                    >
                        Buscar
                    </button>

                    <!-- EXCEL -->
                    <button
                        type="button"
                        @click="window.location.href='{{ route('entradas.excel') }}?search=' + encodeURIComponent(search)"
                        class="bg-green-700 hover:bg-green-800 text-white px-4 py-2 rounded shadow"
                    >
                        Excel
                    </button>

                    <!-- PDF -->
                    <button
                        type="button"
                        @click="window.location.href='{{ route('entradas.pdf') }}?search=' + encodeURIComponent(search)"
                        class="bg-red-700 hover:bg-red-800 text-white px-4 py-2 rounded shadow"
                    >
                        PDF
                    </button>

                    <!-- LIMPIAR -->
                    <a
                        href="{{ route('entradas.index') }}"
                        class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded shadow text-center"
                    >
                        Limpiar
                    </a>

                    <!-- NUEVA -->
                    <a
                        href="{{ route('entradas.create') }}"
                        class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded shadow text-center"
                    >
                        + Nueva
                    </a>

                </div>

            </div>

            <!-- ALERTAS -->
            @if(session('success'))
                <div class="mb-4 p-4 bg-green-100 text-green-700 rounded">
                    {{ session('success') }}
                </div>
            @endif

            <!-- RESULTADO FILTRO -->
            @if(request()->filled('search'))
                <div class="mb-4 p-3 bg-blue-100 text-blue-800 rounded">
                    Buscando: <strong>{{ request('search') }}</strong>
                </div>
            @endif

            <!-- TABLA -->
            <div class="bg-white dark:bg-gray-800 shadow rounded-lg overflow-x-auto">

                <table class="min-w-full text-sm text-left text-gray-600 dark:text-gray-300">

                    <thead class="bg-gray-100 dark:bg-gray-700 text-xs uppercase">
                        <tr>
                            <th class="px-6 py-3">Folio</th>
                            <th class="px-6 py-3">Código</th>
                            <th class="px-6 py-3">Producto</th>
                            <th class="px-6 py-3">Cantidad</th>
                            <th class="px-6 py-3">Orden</th>
                            <th class="px-6 py-3">Fecha</th>
                            <th class="px-6 py-3 text-center">Acciones</th>
                        </tr>
                    </thead>

                    <tbody>
                        @forelse($entradas as $entrada)
                            <tr class="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">

                                <!-- FOLIO -->
                                <td class="px-6 py-3">
                                    <span class="bg-blue-100 text-blue-700 px-2 py-1 rounded text-xs font-semibold">
                                        {{ $entrada->folio }}
                                    </span>
                                </td>

                                <!-- CODIGO -->
                                <td class="px-6 py-3 font-medium text-indigo-600">
                                    {{ $entrada->producto->codigo ?? 'N/A' }}
                                </td>

                                <!-- PRODUCTO -->
                                <td class="px-6 py-3 font-medium">
                                    {{ $entrada->producto->descripcion ?? 'N/A' }}
                                </td>

                                <!-- CANTIDAD -->
                                <td class="px-6 py-3">
                                    {{ $entrada->cantidad }}
                                    {{ optional($entrada->producto)->unidad_medida }}
                                </td>

                                <!-- ORDEN -->
                                <td class="px-6 py-3">
                                    {{ $entrada->orden_compra }}
                                </td>

                                <!-- FECHA -->
                                <td class="px-6 py-3">
                                    {{ \Carbon\Carbon::parse($entrada->fecha_ingreso)->format('d/m/Y') }}
                                </td>

                                <!-- ACCIONES -->
                                @role('admin')
                                <td class="px-6 py-3 text-center space-x-2">

                                    <a href="{{ route('entradas.edit', $entrada->id) }}"
                                        class="bg-yellow-400 hover:bg-yellow-500 text-white px-3 py-1 rounded text-xs">
                                        Editar
                                    </a>

                                    <form action="{{ route('entradas.destroy', $entrada->id) }}"
                                          method="POST"
                                          class="inline-block"
                                          onsubmit="return confirm('¿Eliminar entrada?')">
                                        @csrf
                                        @method('DELETE')

                                        <button type="submit"
                                                class="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded text-xs">
                                            Eliminar
                                        </button>
                                    </form>

                                </td>
                                @endrole

                            </tr>
                        @empty
                            <tr>
                                <td colspan="7" class="text-center py-6 text-gray-500">
                                    No se encontraron registros
                                </td>
                            </tr>
                        @endforelse
                    </tbody>

                </table>

            </div>

        </div>
    </div>
</x-app-layout>